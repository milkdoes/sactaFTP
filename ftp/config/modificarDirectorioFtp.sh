...
cp $RUTA_CONF_VSFTPD $RUTA_CONF_VSFTPD.orig
...
echo "$USUARIO:newpassword" | $CONTRASENA
...
echo -e "\n# Agregar usuario a ruta para permitir acceso." >> $RUTA_CONF_VSFTPD
echo -e "\nuser_sub_token=$USER" >> $RUTA_CONF_VSFTPD
echo -e "\nlocal_root=$DIR_FTP" >> $RUTA_CONF_VSFTPD
echo -e "\n" >> $RUTA_CONF_VSFTPD

# Agregar lista de puertos pasivos.
echo -e "\n# Agregar lista de puertos pasivos." >> $RUTA_CONF_VSFTPD
echo -e "\npasv_min_port=40000" >> $RUTA_CONF_VSFTPD
echo -e "\npasv_min_port=50000" >> $RUTA_CONF_VSFTPD
echo -e "\n" >> $RUTA_CONF_VSFTPD

# Permitir usuarios acceso al servidor FTP cuando esten en la lista.
echo -e "\nuserlist_enable=YES" >> $RUTA_CONF_VSFTPD
echo -e "\nuserlist_file=$RUTA_USUARIOS_PERMITIDOS" >> $RUTA_CONF_VSFTPD
echo -e "\nuserlist_deny=NO" >> $RUTA_CONF_VSFTPD
echo -e "\n" >> $RUTA_CONF_VSFTPD
